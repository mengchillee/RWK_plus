...
def create_dataset(cfg): 
    dataset = PygGraphPropPredDataset(root='data', name='ogbg-molpcba') 
...
def train(train_loader, model, optimizer, device):
    ...
    for data in train_loader:
        ...
        loss = loss_func(model(data)[mask], y[mask].float())
        loss.backward()
...
def test(loader, model, evaluator, device):
    ...
    return result_dict[evaluator.eval_metric]


if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('train/config/molpcba.yaml')
    cfg = update_cfg(cfg)
