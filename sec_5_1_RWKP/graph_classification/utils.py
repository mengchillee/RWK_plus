...
def load_data(ds_name, task, use_node_labels=False, use_node_attri=False):    
    ...
    class_labels = np.loadtxt("datasets/%s/%s_graph_labels.txt" % (ds_name, ds_name), delimiter=',', dtype=np.float)[:, task]

    return adj, features, class_labels

def load_data_ori(ds_name, use_node_labels=False, use_node_attri=False):    
    graph_indicator = np.loadtxt("datasets/%s/%s_graph_indicator.txt"%(ds_name,ds_name), dtype=np.int64)
    _, graph_size = np.unique(graph_indicator, return_counts=True)
...
def generate_batch_train(adj):
    N = len(adj)
    index = np.array(range(N), dtype=np.int32)

    indices, values, graph_indicator_batch = [], [], []
    k_size = adj.shape[1]
    for i in range(adj.shape[0]):
        S_indices = adj[i].coalesce().indices()
        S_values = adj[i].coalesce().values()
        indices.append(S_indices + k_size * i)
        values.append(S_values)
        graph_indicator_batch.append(i)

    S_indices, S_values = torch.cat(indices, dim=1), torch.cat(values)
    graph_indicator_batch = torch.tensor(graph_indicator_batch).repeat_interleave(k_size).to(adj.device)
    return S_indices, S_values, graph_indicator_batch
