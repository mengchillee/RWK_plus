...
def load_data(dataset):
    ...
    max_deg = int(np.max(A.sum(0)))
    for i in range(graph_size.size):
        a = A[start_idx:start_idx + graph_size[i], start_idx:start_idx + graph_size[i]]
        x = X[start_idx:start_idx + graph_size[i], :]
        deg = np.zeros((graph_size[i], max_deg))
        for j, d in enumerate(np.array(a.sum(0)).reshape(-1)):
            if d != 0:
                deg[j, int(d-1)] = 1
        adj.append(a)
        features.append(np.concatenate([x, deg], axis=1))
    ...
    return adj, features, class_labels, feat_dim


def generate_batches_(adj, features, label, batch_size, graph_pooling_type, device, shuffle, seed=None):
    np.random.seed(seed)
...
def generate_batches_kernel(adj, features, label, batch_size):
    N = len(label) # the number of graphs 
    index = np.array(range(N), dtype=np.int32) # shape (84, )
    n_batches = ceil(N/batch_size)
    ...
    for i in range(0, N, batch_size):
        ...
        adj_batch = lil_matrix((n_nodes, n_nodes))
        features_batch = lil_matrix((n_nodes, features[0].shape[1]))
        graph_indicator_batch = np.zeros(n_nodes)
        label_batch = np.zeros(n_graphs)
        ...
        label_lst.append(label_batch)

    return adj_lst, features_lst, graph_indicator_lst, label_lst, n_batches
