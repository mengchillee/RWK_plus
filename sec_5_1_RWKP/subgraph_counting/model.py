...
class RW_layer(nn.Module):  
    def __init__(self, n_kernels, hidden_dim=None, max_step=1, size_graph_filter=10, color_matching=False, seed=0):
        ...
        self.bns = nn.ModuleList(nn.BatchNorm1d(n_kernels) for _ in range(max_step))
    ...
    def forward(self, adj, features, idxs):  
        ...
        if not self.color_matching:
            ...
            for i in range(self.max_step):
                ...
                t = self.bns[i](t.reshape(-1, self.n_kernels)).reshape(t.shape[0], self.size_graph_filter, -1, self.n_kernels)
                t = torch.sigmoid(t)
            ...
            for i in range(self.max_step):
                ...
                y = y0 * y 
                t = torch.mean(torch.sigmoid(self.bns[i](y.reshape(-1, self.n_kernels)).reshape(y.shape[0], self.size_graph_filter, -1, self.n_kernels)), dim=[1, 2])
                y = y0 * y 
                out.append(t)
